...
Renames applied in v2.0 relative to v1.0.1 (notation only — no semantic changes):

| v1.0.1 | v2.0 | Object |
|---|---|---|
| Φ, Φ_d, Φ_eff | 𝔉, 𝔉_d, 𝔉_eff | Emergent Function, declared/effective forms (frees Φ for the OST emergent function and the TE collapse exception) |
| Δ, Δ_S | Δ_𝔉 | The Gap 𝔉_d − 𝔉_eff |
| K | 𝒦_p | Pure Knowledge (frees K = Context, TE primitive) |
| K_real | 𝒦_r | Realized Knowledge |
| I, I₁–I₁₀, I_new | ι, ι₁–ι₁₀, ι_new | Invariant and library (ι-family programme-wide; frees I = Identity) |
| C (Controphase) | C_φ | Controphase marker (frees C = Coherent Content, TE primitive) |
| D (Domain) | 𝔻 | Domain (frees D = Decoherent Space) |
| τ | τ_ph | Temporal Phase label (bare τ = Pulsational Function at programme level) |
| λ | λ_L | Lyapunov exponent (λ = Dominant Vector in TE/PA) |
| ω | ω_att | Attractor end-state (ω = Evolutionary Convergence in TE) |

Unchanged by design: S (Strip), π (Re-contextualisation, with π_v), σ (Singularity — OST-aligned), R (Relational Field — OST-aligned), ρ, θ, Σ_src, U (Expressive Functor — categorical dual of OCT's forgetful functor U, per OCT theorem D03), O, V. **κ (Coherence index) is retained within SA scope by canon declaration** — distinct from PA's ⟨𝓚⁵⟩ aggregate and TE's pairwise κ. The three lines quoting the TE collapse equation E = Φ(C, I, K) retain TE notation by design (declared exception).
