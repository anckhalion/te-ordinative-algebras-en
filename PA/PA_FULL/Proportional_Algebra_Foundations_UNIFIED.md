...
**Author:** Fabio Ghioni
**Programme:** Technology of Expressions — Ordinative Sciences
...
$$\mathcal{L}' = \mathcal{L} \quad \text{under } \psi \to e^{i\alpha(x)}\psi$$
...
$$I_{n+1} = \mathcal{U}(I_n, E_n^{grief})$$
...
$$\rho(C_{genome}, I_{disease-pattern}) \geq \theta \; ?$$
...
$$E_{treated} = \Phi(C_{genome}, I_{organism}, K_{drug})$$
